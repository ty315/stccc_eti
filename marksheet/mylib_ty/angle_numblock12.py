...
def rotatefile(i):
    ...
    cv2.imwrite(f'{i:05d}_angle.jpg',rot_img)
...
# 手書き数字領域の切り取り、数字毎のファイル作成
def trim_tegaki(i):
    anglefile = f'{i:05d}_angle.jpg'
    img = Image.open(anglefile)
    img_crop = img.crop((1,500, 1800, 3200))
    img_crop.save(f'{i:05d}_crop.jpg', quality=95)
    img_tegaki = cv2.imread(f'{i:05d}_crop.jpg')
    ...
    cv2.imwrite(f'{i:05d}_crop.jpg',img_tegaki)
    plt.imshow(img_tegaki, cmap = "gray")
    plt.show()
