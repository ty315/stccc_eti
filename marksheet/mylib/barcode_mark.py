...
def barcodemark(i):
    ...
    cv2.imwrite(f'./image/{i:05d}_no.jpg',img_no_no, [cv2.IMWRITE_JPEG_QUALITY, 100])
    img_ocr = Image.open(f'./image/{i:05d}_no.jpg')
    # OCRの実行
    ...
    display(df3)
    os.remove('res.png')
    df3.to_csv(f'./csv/{i:05d}_mark.csv')
    print("mark.csv was created")